...
{
    public static int MaxStackAllocSize { get; set; } = 4096 << (sizeof(nuint) >= 8 ? 2 : 0);

    /// <summary>
    /// Determines whether to use a stack or a heap allocation by passing a generic type and the element count.
    /// The default maximum stack allocation size is set to 16384 bytes for 64-bit processes and to 4096 bytes for 32-bit processes.
