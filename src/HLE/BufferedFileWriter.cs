...
public readonly struct BufferedFileWriter : IEquatable<BufferedFileWriter>
{
    ...
    public ValueTask WriteBytesAsync(ReadOnlyMemory<byte> fileBytes)
        => WriteBytesAsync(fileBytes, false);

    public void WriteChars(ReadOnlySpan<char> fileContent, Encoding fileEncoding)
        => WriteChars(fileContent, fileEncoding, false);

    // ReSharper disable once InconsistentNaming
    public ValueTask WriteCharsAsync(ReadOnlyMemory<char> fileContent, Encoding fileEncoding)
        => WriteCharsAsync(fileContent, fileEncoding, false);

    public void AppendBytes(ReadOnlySpan<byte> fileBytes)
        => WriteBytes(fileBytes, true);

    // ReSharper disable once InconsistentNaming
    public ValueTask AppendBytesAsync(ReadOnlyMemory<byte> fileBytes)
        => WriteBytesAsync(fileBytes, true);

    public void AppendChars(ReadOnlySpan<char> fileContent, Encoding fileEncoding)
        => WriteChars(fileContent, fileEncoding, true);

    // ReSharper disable once InconsistentNaming
    public ValueTask AppendCharsAsync(ReadOnlyMemory<char> fileContent, Encoding fileEncoding)
        => WriteCharsAsync(fileContent, fileEncoding, true);
