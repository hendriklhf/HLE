...
public sealed class ThrowHelperTest
{
    ...
    [Fact]
    public void ThrowTypeNotSupportedTest()
    {
        NotSupportedException exception = Assert.Throws<NotSupportedException>(ThrowHelper.ThrowTypeNotSupported<Stream>);
        Assert.Contains(typeof(Stream).ToString(), exception.Message, StringComparison.Ordinal);
    }
